...
--
/*!50003 DROP PROCEDURE IF EXISTS `insertOrganization` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertOrganization`()
BEGIN
insert into `bankoomorganization`.`organization`(idOrganization, `password`, `comercialName`, idState) values ('CFE SA CV','hola', 'CFE', 1);
END ;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `new_procedure`(`@org` int)
BEGIN
select `@org` as result;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `transaction`(_idAccountSender int, _idAccountReciever int, _amount decimal(7,2), _concept varchar(20))
BEGIN
  ...
  insert into transfer(idAccountSender, idAccountReciever, amount, concept , dateOfTransfer) values(_idAccountSender, _idAccountReciever, _amount, _concept, curdate());
  update `account` set balance = balance-_amount where idAccount = _idAccountSender;
  update `account` set balance = balance+_amount where idAccount = _idAccountReciever;
...
BEGIN
  ...
  IF (SELECT balance FROM `account` WHERE idAccount = _idAccountSender) - _amount < 0 THEN
      SELECT 'Saldo insuficiente' AS message;
  ELSE
    INSERT INTO transfer(idAccountSender, idAccountReceiver, amount, concept, dateOfTransfer) VALUES (_idAccountSender, _idAccountReceiver, _amount, SUBSTRING(_concept, 1, 20), CURRENT_TIMESTAMP);
    UPDATE `account` SET balance = balance - _amount WHERE idAccount = _idAccountSender;
    UPDATE `account` SET balance = balance + _amount WHERE idAccount = _idAccountReceiver;
    SELECT 'Transferencia Realizada' AS message;
  END IF;
END ;;
DELIMITER ;
